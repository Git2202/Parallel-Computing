...
int main(){
    ...
    mergeSortSeq(arr_seq,0,n-1);
    double time_seq=omp_get_wtime()-start_time;
    printf("Sequential merge sort time: %f in seconds\n",time_seq);
    
    start_time=omp_get_wtime();
    mergeSortParallel(arr_par,0,n-1);
    double time_par = omp_get_wtime()-start_time;
    printf("Parallel merge sort time: %f in seconds\n", time_par);

    printf("\n Diff between seq and par: %f seconds\n",time_seq-time_par);
