...
int main(){
    ...
    int seq_prime_count=0;
    for(int i=1; i<=n;i++){
        if (is_prime(i)) seq_prime_count++;
    }
    double time_seq=omp_get_wtime() - start_time;
    printf("\nSequential: Found %d primes in %f seconds\n",seq_prime_count, time_seq);

    start_time=omp_get_wtime();
    int par_prime_count=0;
    #pragma omp parallel for reduction(+:par_prime_count) schedule(dynamic)
    for(int i=1; i<=n;i++){
        if (is_prime(i)) par_prime_count++;
    }
    double time_par = omp_get_wtime() - start_time;
    printf("Parallel: Found %d primes in %f seconds\n",par_prime_count,time_par);
